...
export const UsersManagement = () => {
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await server.requestPut(`/admin/user/${editingUser.id}/`, editFormData);
      setShowEditModal(false);
      setEditingUser(null);
      setEditFormData({ username: "", full_name: "", role: "STUDENT" });
      fetchUsers();
    } catch (error) {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-800">{c.t("Foydalanuvchilar Boshqaruvi")}</h2>
        <button
